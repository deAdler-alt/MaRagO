...
with col_donut:
    # Donut: priorytet_smart per aktywny samolot
    priority_counts = (
...
if top10.empty:
    ...
    def _style_row(row):
        ...
        if "zaległe" in p:
            return [f"background-color:#7f1d1d; color:#fef2f2"] * len(row)
        if "TERAZ" in p:
            return [f"background-color:#991b1b; color:#fef2f2"] * len(row)
        if "6 mies" in p:
            return [f"background-color:#78350f; color:#fef3c7"] * len(row)
        return [""] * len(row)
